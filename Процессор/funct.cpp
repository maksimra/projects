...
void f_read (size_t size, FILE *fp, char** buffer_ptr, enum error* Error)
{
    ...
    *buffer_ptr = buffer;
    ...
    for (size_t i = 0; i < size && (buffer[i] = fgetc (fp)) != EOF; i++)
        continue;
...
void error_output (enum error Error)
{
    switch (Error)
    {
        ...
        case ERROR_STAT:
            ...
            break;
        case ERROR_STRCHR:
            printf ("Ошибка использования функции strchr :(\n");
            break;
        case ERROR_CALLOC:
            printf ("Ошибка использования функции calloc :(\n");
            break;
        default:
            printf ("Кто-то сломал enum  с ошибками!!!\n");
    }
}
void bubble_sort (Lines* onegin, size_t line_counter)
{
    for (size_t pass = 0; pass < line_counter - 1; pass++)
    {
        int nswap = 0;
        for (size_t i = 0; i < line_counter - pass - 1; i++)
        {
...
// {
//     return my_strcmp((char*) s1, (char*) s2);
// }
